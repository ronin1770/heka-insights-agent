...
Build a bundled Linux executable and wrap it in a Debian package for Ubuntu
`amd64`.

## Packaging Path

- build script: `packaging/debian/build_deb.sh`
- service unit: `packaging/debian/heka-insights-agent.service`
- maintainer scripts:
...
```bash
./packaging/debian/build_deb.sh <version> "<maintainer>"
```

Example:

```bash
./packaging/debian/build_deb.sh 0.1.0 "Heka Insights Agent <munir.farhan@gmail.com>"
```
