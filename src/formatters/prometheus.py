"""Prometheus text formatter for raw collector payloads."""
...
class _FamilyMeta:
    """Metadata for one Prometheus metric family."""
...
class PrometheusFormatter:
    """Render CPU/memory/disk collector payloads in Prometheus text format."""
    ...
    def format(
        ...
        *,
        ...
        """Convert raw collector payloads into one Prometheus text document."""
    ...
    def _format_timestamp(timestamp_ms: int | float | None) -> str | None:
        if timestamp_ms is None:
            return None
        return str(int(float(timestamp_ms)))
